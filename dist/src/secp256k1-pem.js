...
class Secp256k1PEM {
  ...
  static fromSeed(seed) {
    ...
    const defaultKey = secpPEM._hdKey.derive(DEFAULT_HD_PATH + '0');
    secpPEM._rawPEM = secp256k1.utils.concatBytes(
      PEM_STATIC_1,
      defaultKey.privateKey,
      PEM_STATIC_2,
      secp256k1.Point.fromPrivateKey(defaultKey.privateKey).toRawBytes(false)
    );
  ...
  static fromExtendedKey(base58key) {
    ...
    secpPEM._rawPEM = secp256k1.utils.concatBytes(
      PEM_STATIC_1,
      secpPEM._hdKey.privateKey,
      PEM_STATIC_2,
      secp256k1.Point.fromPrivateKey(secpPEM._hdKey.privateKey).toRawBytes(false)
    );
    return secpPEM;
  }
  getRawPAM() {
